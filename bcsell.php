<?
	...
	if ($q !== false) {
		...
		// BitcoinMarket.com
			$data = file("markets/bitcoinmarket/data.BID.PayPalUSD.csv", FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
		...
		array_multisort($asks["price"], SORT_DESC, $asks["quantity"], $asks["market"]);
		...
		if (preg_match("/^(\\$|US\\$|USD|USD\\$)([0-9.]*)$/i", $qu, $matches)) {
			...
			if (is_numeric($amount)) {
				...
				echo $out."Total amount for \$".($amount - $money)." is BTC$bitcoins";
			}
			...
			if (is_numeric($amount)) {
				...
				echo $out."Total amount for BTC$total is \$$price";
			}
			else if ($i == "irc") echo "I do not understand \"$q.\"";
		}
...
?>Sell: <input id="inputsell" type="text"> (Input in USD, $, BTC or Bitcoins)
  <pre id="outputsell" style="max-height: 200px; overflow: auto;"></pre>
  <script type="text/javascript">
	$("#inputsell").focus();
	$("#inputsell").keyup(function() {
		var url = "bcsell.php?q="+encodeURIComponent($("#inputsell").val());
		$.get(url, function(data) {
			if (data.length > 0) $('#outputsell').html(data);
			else $('#outputsell').html("");
		});
	});
  </script>
<?
